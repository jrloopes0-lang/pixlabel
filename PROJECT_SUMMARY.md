...
- **Authentication**: Sessão baseada em cookies para ambientes próprios
- **Session**: express-session (plugável com store externo)
...
└── middleware/
    └── auth.ts      # Middleware de autenticação (pluggable OIDC/custom)
...
- `GET /auth/login` - Generic OIDC/OAuth2 redirect (issuer/client configured via env vars)
- `GET /auth/callback` - Handles the OIDC response and exchanges the code for a session
...
### ✅ Implementado
- **Estoque Core**: CRUD de itens, unidades, estoque e movimentações com fallback para mocks
- **Autenticação**: Tela de login com copy genérico para OIDC e backend pronto para sessões via provider configurável
- **PMS Base**: CRUD e mocks para pacientes, pareceres, evoluções e dispensações
- **UI/UX**:
  - Dashboard e páginas módulo em React Router
  - Sidebar persistente com navegação
  - Layout com tokens de design e componentes base (cards, tabelas, stat grid)
...
# Database operations
npm run db:push        # Sync schema (Postgres/Neon)
...
OIDC_ISSUER=...               # Provedor OIDC genérico
OIDC_CLIENT_ID=...            # Client ID
OIDC_CLIENT_SECRET=...        # Client secret
OIDC_REDIRECT_URI=...         # Callback URL apontando para /auth/callback
```
...
- **OIDC/OAuth2**: Documentação do provedor configurado (ex.: Auth0, Keycloak)
...
**Status**: MVP consolidado com mocks e pronto para plugar OIDC + Postgres
